...
`username` and `icon_emoji` are optional. They customize the Slack app identity
...
`ctx.agent_turn(...)` accepts optional `model` and `reasoning` kwargs. They ride
the turn exactly like the Slack `--model` / `-rsn` flags: `model` selects the
model within the harness, and `reasoning` sets the Codex reasoning effort
(`none`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`). Codex always uses the
deployment's configured OpenAI-compatible gateway; provider selection is not
supported. `reasoning` also accepts the `reasoning_effort` and `effort` aliases.
When a kwarg is omitted the deployment/baked harness default stands — dispatched
turns are no longer pinned to the deployment default.
