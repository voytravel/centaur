...
## Enable Allowlisted Slack Direct Messages

Centaur ignores direct messages by default. To enable one-to-one direct
messages for specific Slack members, subscribe the Slack app to `message.im`,
grant `im:history`, enable its writable App Home Messages tab, and set their
member IDs in the chart value:

```yaml
slackbotv2:
  directMessageUserAllowlist: "U01234567,U07654321"
```

Only members in this allowlist can start a DM session; group DMs and messages
from every other member remain ignored. Configure the resulting DM principals
with the same least-privilege roles as any other Slack-derived principal.
