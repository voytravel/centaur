...
| `automation.enabled` | Helm value, default `false`. | Enables Console-backed declarative GitHub and Linear automation policies. The shared secret source must contain `CENTAUR_AUTOMATION_INGRESS_TOKEN`. |
...
| `OPENAI_BASE_URL` | `apiRs.extraEnv`; default `https://api.openai.com/v1`. | OpenAI-compatible Responses API base URL for every OpenAI-compatible call: Codex, Nanocodex, generated session titles, activity summaries, Slack's optional message-override classifier, and company-context embeddings. API-RS passes it into Codex sandboxes, whose startup config renders it as Codex's user-level gateway provider, and derives iron-proxy's `OPENAI_API_KEY` host scope from it. |
...
| `SLACKBOT_DIRECT_MESSAGE_USER_ALLOWLIST` | `slackbotv2.directMessageUserAllowlist`. | Comma-separated Slack member IDs allowed to start one-to-one direct-message sessions. Empty or absent ignores DMs. |
